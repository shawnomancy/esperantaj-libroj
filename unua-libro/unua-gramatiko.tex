...
\begin{tblr}{YYYYYY}
\SetRow{font=\Large} A a, & B b, & C c, & Ĉ ĉ, & D d, & E e,  \\ 
\SetRow{font=\scriptsize} \emph{a} as in ``last'' & \emph{b} as in ``be'' & \emph{ts} as in ``wits'' & \emph{ch} as in ``church'' & \emph{d} as in ``do'' & \emph{e} as in ``make'' \\
\SetRow{font=\Large} F f, & G g, & Ĝ ĝ, & H h, & Ĥ ĥ, & I i, \\
\SetRow{font=\scriptsize} \emph{f} as in ``fly'' & \emph{g} as in ``gun'' & \emph{j} as in ``join'' & \emph{h} as in ``half'' & strongly aspirated h, \emph{ch} in ``loch'' (Scotch) & \emph{i} as in ``marine''  \\
\SetRow{font=\Large} J j, & Ĵ ĵ, & K k, & L l, & M m, & N n,  \\
\SetRow{font=\scriptsize} \emph{y} as in ``yoke'' & \emph{z} as in ``azure'' & \emph{k} as in ``key'' & \emph{l} as in ``line'' & \emph{m} as in ``make'' & \emph{n} as in ``now'' \\
\SetRow{font=\Large} O o, & P p, & R r, & S s, & Ŝ ŝ, & T t,   \\
\SetRow{font=\scriptsize} \emph{o} as in ``not'' & \emph{p} as in ``pair'' & \emph{r} as in ``rare'' & \emph{s} as in ``see'' & \emph{sh} as in ``show'' & \emph{t} as in ``tea'' \\
\SetRow{font=\Large} & U u, & Ŭ ŭ, & V v, & Z z. \\
\SetRow{font=\scriptsize} & \emph{u} as in ``bull'' & \emph{u} as in ``mount'' (used in diphthongs) & \emph{v} as in ``very'' & \emph{z} as in ``zeal'' 
\end{tblr}
\end{center}
