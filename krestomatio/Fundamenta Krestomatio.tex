...
\begin{center}
 ...
 \csfont {\small
{\footnotesize FRANCUJO.} — HACHETTE et C\textsuperscript{ie}, {\it PARIS.} \\
\rule[0.9ex]{13mm}{0.4pt} \\
{\footnotesize ANGLUJO.} – « REVIEW of REVIEWS », {\footnotesize\it LONDON} \\
{\footnotesize DANUJO.} — ANDR.-FRED. HÖST \& SÖN, {\footnotesize\it KJOBENHAVN.} \\
{\footnotesize GERMANUJO.} — MÖLLER \& BOREL, {\footnotesize\it BERLIN.} \\
{\footnotesize HISPANUJO.} — J. ESPASA, {\footnotesize\it BARCELONA.} \\
{\footnotesize ITALUJO.} — RAFFAELO GIUSTI, {\footnotesize\it LIVORNO.} \\
{\footnotesize POLUJO.} — M. ARCT, {\footnotesize\it WARSZAWA.} \\
{\footnotesize SVEDUJO.} — ESPERANTOFÖRENING, {\footnotesize\it STOCKHOLM.} 
}
...
\label{novajvestoj}
\input{003_la_novaj_vestoj_de_la_regxo}

\section{Aleksandro Macedona}
\label{aleksandro}
\input{004_aleksandro_macedona.tex}

\section{Kiu estas la plej bona amiko de la viro?}
\label{plejbona}
\input{005_kiu_estas_la_plej_bona_amiko_de_la_viro.tex}

\section{La deveno de la virino}
\label{deveno}
\input{006_la_deveno_de_la_virino.tex}

\section{La naski\^go de la tabako}
\label{tabako}
\input{007_la_naskigxo_de_la_tabako.tex}

\section{Karagara}
\label{karagara}
\input{008_karagara.tex}

\section{La virineto de maro}
\label{virineto}
\input{009_la_virineto_de_maro.tex}
...
\label{metiisto}
\input{012_la_hejmo_de_la_metiisto.tex}

\section{La forgesita pipo}
\label{pipo}
\input{013_la_forgesita_pipo.tex}

\section{Arturo}
\label{arturo}
\input{014_arturo.tex}

\newpage % next section header comes up orphaned otherwise

\section{La nigra virino}
\label{nigra}
\input{015_la_nigra_virino.tex}

\section{La karaj braceletoj}
\label{braceletoj}
\input{016_la_karaj_braceletoj.tex}

\section{Nur unu vorton!}
\label{unuvorton}
\input{017_nur_unu_vorton.tex}

\section{La porcio da glacia\^{\j}o}
\label{porcio}
\input{018_la_porcio_da_glaciajxo.tex}
...
\section{Bagateloj}
\label{bagateloj}
\input{019_bagateloj.tex}

\section{Fingra kalendaro}
\label{fingra}
\input{020_fingra_kalendaro.tex}

\section{El la po\^sto}
\label{posxto}
\input{021_el_la_posxto.tex}

\section{La lo\^gejoj de la termitoj}
\label{termitoj}
\input{022_la_logxejoj_de_la_termitoj.tex}

\section{Korespondado komerca}
\label{komerca}
\input{023_korespondado_komerca.tex}

\section{Kronika katara konjunktivito}
\label{kronika}
\input{024_kronika_katara_konjunktivito.tex}

\section{La sunhorlo\^go en Dijon}
\label{dijon}
\input{025_la_sunhorlogxo_en_dijon.tex}
...
\label{unualibro}
\input{026_el_la_unua_libro_de_la_lingvo_esperanto.tex}

\section{Plena gramatiko de Esperanto}
\label{plena}
\input{027_plena_gramatiko_de_esperanto.tex}
...
\label{laespero}
\input{030_la_espero.tex}

\subsection{La vojo}
\label{lavojo}
\input{031_la_vojo.tex}

\subsection{Al la fratoj}
\label{allafratoj}
\input{032_al_la_fratoj.tex}

\subsection{Mia penso}
\label{miapenso}
\input{033_mia_penso.tex}

\subsection{Ho mia kor'}
\label{homiakor}
\input{034_ho_mia_kor.tex}

\subsection{La vojevodo}
\label{vojevodo}
\input{035_la_vojevodo.tex}

\subsection{La rozeto}
\label{rozeto}
\input{036_la_rozeto.tex}

\subsection{En nord' unu pino\dots}
\label{unupino}
\input{037_en_nord_unu_pino.tex}

\subsection{En son\^go\dots}
\label{songxo}
\input{038_en_songxo.tex}

\subsection{Lorelej'}
\label{lorelej}
\input{039_lorelej.tex}

\subsection{Kanto de studentoj}
\label{studentoj}
\input{040_kanto_de_studentoj.tex}

\subsection{Al la re\^go}
\label{regxo}
\input{041_al_la_regxo.tex}

\subsection{Nokta kanto de soldato}
\label{soldato}
\input{042_nokta_kanto_de_soldato.tex}

\subsection{Kanto de l' ligo}
\label{ligo}
\input{043_kanto_de_l_ligo.tex}

\subsection{La kapelo}
\label{kapelo}
\input{044_la_kapelo.tex}

\subsection{La gaja migranto}
\label{migranto}
\input{045_la_gaja_migranto.tex}

\subsection{La vojo}
\label{lavojo2}
\input{046_la_vojo.tex}

\addcontentsline{toc}{section}{Léo Belmont}

\subsection{Nokt' en la koro}
\label{noktenlakoro}
\input{047_nokto_en_la_koro.tex}

\subsection{Al brusto, al min\dots}
\label{albrusto}
\input{048_al_brusto_al_min.tex}

\addcontentsline{toc}{section}{V. Devjatnin}

\subsection{Printempo}
\label{printempo}
\input{049_printempo.tex}

\subsection{\^Sipeto}
\label{sxipeto}
\input{050_sxipeto.tex}

\subsection{Sciuro kaj papago}
\label{sciuro}
\input{051_sciuro_kaj_papago.tex}

\subsection{An\^gelo}
\label{angxelo}
\input{052_angxelo.tex}

\subsection{Husaro}
\label{husaro}
\input{053_husaro.tex}

\subsection{Infano de zorgo}
\label{infano}
\input{054_infano_de_zorgo.tex}

\subsection{Espero}
\label{espero}
\input{055_espero.tex}

\subsection{Garantio}
\label{garantio}
\input{056_garantio.tex}
...
\label{tri}
\input{063_tri_budrysoj.tex}

\vspace*{-5ex} %

\subsection{Reva\^{\j}o}
\label{revajxo}
\input{064_revajxo.tex}

\vspace*{-5ex} %

\subsection{Excelsior}
\label{excelsior}
\input{065_excelsior.tex}
\subsection{La pluva tago}
\label{pluva}
\input{066_la_pluva_tago.tex}
\subsection{La sago kaj la kanto}
\label{sago}
\input{067_la_sago_kaj_la_kanto.tex}
\subsection{Sonoriloj de vespero}
\label{sonoriloj}
\input{068_sonoriloj_de_vespero.tex}

\addcontentsline{toc}{section}{E. Haller}
\subsection{Mi rakontis}
\label{rakontis}
\input{069_mi_rakontis.tex} % had to add newpage inside this
...
\label{riprocxu}
\input{084_ne_riprocxu_la_sorton.tex}
\subsection{Revo}
\label{revo}
\input{085_revo.tex}
...
\label{hamleto}
\input{098_el_hamleto.tex}

\addcontentsline{toc}{section}{A. Kofman}
\subsection{El Iliado}
\label{iliado}
\input{099_el_Iliado.tex}
